...
export default function InstaDesignerPage() {
  ...
  // ── 필터 (warmth 기본값 0 — ColorMatrix 초기 미실행) ──
  ...
  const [warmth,     setWarmth]     = useState(0);   // ← 0으로 수정
  ...
  }, [brightness, contrast, saturation, warmth]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  function initFabric(r: Ratio = ratio) {
    ...
    fabricRef.current = new Fab.Canvas(canvasRef.current, {
      width: w, height: h, backgroundColor: MINT,
    });
  ...
  const undo = useCallback(() => {
    ...
    setRedoStack(r => { const n=[...r, current]; setCanRedo(true); return n; });
  ...
  const loadImage = useCallback((file: File) => {
    ...
    reader.onload = e => {
      Fab.Image.fromURL(
        ...
        (img: any) => {
          if (!img) { showToast("이미지 로드 실패 — 다시 시도해주세요"); return; }
          const { w, h } = getDims();
          // 스케일 초기화 후 재계산
          ...
          fabricRef.current.setBackgroundColor(MINT, () => {});
  ...
  function applyFilter(img?: any) {
    ...
    // ColorMatrix: 지원 여부 확인 + try-catch
    if (warmth !== 0 && Fab.Image.filters.ColorMatrix) {
      try {
        ...
        console.warn("[applyFilter] ColorMatrix 미지원:", err);
      }
    }
    ...
    try {
      ...
      console.warn("[applyFilter] applyFilters 에러:", err);
      target.filters = [];
    }
    fabricRef.current?.renderAll();
  }
    ...
    if (tmpl === "bottom") {
      for (let i = 0; i < 8; i++) {
        fabricRef.current.add(new Fab.Rect({
          ...
          fill: `rgba(0,0,0,${((i / 8) * alpha).toFixed(2)})`,
  ...
  // ── 로고 (렌즈 심볼 + PHOTO CLINIC) ──────────────────
  function addLogo(cw: number, ch: number) {
    removeByName("logo");
    const Fab = getFab();
    if (!Fab || !fabricRef.current) return;
    const fc = fabricRef.current;
    const r1 = 9, r2 = 6, r3 = 3;
    const lx = cw - 14 - 80;
    const ly = ch - 14 - 11;
    const cx = lx + r1, cy = ly + r1;
    ...
    fc.add(new Fab.Text("PHOTO", {
      left: cx + r1 + 3, top: cy - 5,
      fontSize:8, fill:"#E85D2C",
      fontFamily:"'Noto Sans KR',sans-serif", fontWeight:"700",
      letterSpacing:1.5, selectable:true, name:"logo",
    }));
    // CLINIC (틸)
    fc.add(new Fab.Text("CLINIC", {
      left: cx + r1 + 3 + 34, top: cy - 5,
      fontSize:8, fill:"#155855",
      fontFamily:"'Noto Sans KR',sans-serif", fontWeight:"700",
      letterSpacing:1.5, selectable:true, name:"logo",
    }));
  }
  ...
  function placeText(main: string, sub?: string) {
    const Fab = getFab();
    if (!Fab || !fabricRef.current) { showToast("에디터 준비 중"); return; }
    ...
    const shadow = template !== "overlay-teal" && template !== "minimal"
      ? "rgba(0,0,0,0.5) 0 2px 8px" : undefined;

    if (main) fc.add(new Fab.IText(main, {
      left: cw/2, top: topM, originX:"center",
      fontSize, fill: textColor,
      fontFamily: `'${fp.display}',serif,sans-serif`,
      fontWeight:"600", textAlign, width: cw*0.86,
      lineHeight: lineH, charSpacing: letterSp*10, shadow, name:"main-text",
    }));
    if (sub) fc.add(new Fab.IText(sub, {
      ...
      fontWeight:"400", textAlign, width: cw*0.86,
      opacity: 0.75, lineHeight: lineH, charSpacing: letterSp*10, shadow, name:"sub-text",
    }));
    fabricRef.current.renderAll();
    saveUndoState();
  ...
  function addShape(type: "rect"|"circle"|"line"|"triangle") {
    ...
    if (type === "line")     obj = new Fab.Line([0,0,cw*0.4,0],
      { left:cw/2, top:ch*0.6, stroke:accentColor, strokeWidth:1.5,
  ...
  const addFreeText = () => {
    ...
    const obj = new Fab.IText("텍스트", {
      ...
      fontSize, fill:textColor,
      fontFamily:`'${fp.display}',serif,sans-serif`, fontWeight:"600",
    });
    fabricRef.current.add(obj);
    fabricRef.current.setActiveObject(obj);
    fabricRef.current.renderAll();
  ...
  // ── 로컬 변수 (JSX용) ─────────────────────────────────
  // ※ const tmpl = template 같은 선언을 JSX 렌더 밖에서 하면 안 됨
  const { w: cw, h: ch } = getDims();
  const fc = fabricRef.current; // placeText 내부에서만 사용
  ...
  const iStyle: React.CSSProperties = {
    ...
    background:"#222", color:txt, outline:"none",
  };
  const bStyle = (active?: boolean): React.CSSProperties => ({
    border:`1px solid ${active ? accent : border}`,
    borderRadius:7, background: active ? "#2A1A10" : "#222",
    color: active ? accent : txt,
    fontSize:11, fontWeight:600, cursor:"pointer",
    padding:"6px 10px", fontFamily:"inherit", transition:"all .15s",
  });

  // ── JSX ───────────────────────────────────────────────
  return (
    <div style={{ display:"flex", flexDirection:"column", minHeight:"100vh", background:"#111109", color:txt }}>

      {/* 툴바 */}
      <div style={{ background:"#151513", borderBottom:`1px solid ${border}`, padding:"8px 16px", display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
        {/* 로고 */}
        <div style={{ display:"flex", alignItems:"center", gap:8, marginRight:12 }}>
          <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
            <circle cx="9" cy="9" r="8" fill="#E85D2C"/>
            <circle cx="9" cy="9" r="8" fill="#155855" clipPath="url(#tlr)"/>
            <defs><clipPath id="tlr"><rect x="9" y="0" width="9" height="18"/></clipPath></defs>
            <circle cx="9" cy="9" r="5.5" fill="#EB8F22"/>
            <circle cx="9" cy="9" r="5.5" fill="#569082" clipPath="url(#tlr)"/>
        ...
        {!fabricReady && (
          <span style={{ fontSize:11, color:"#C8860A", background:"#2A2000", padding:"3px 10px", borderRadius:6 }}>
            초기화 중...
          </span>
        )}
        ...
        <button onClick={undo} disabled={!canUndo} title="실행 취소 (⌘Z)"
          style={{ ...bStyle(false), opacity: canUndo?1:0.35 }}>↩ 취소</button>
        <button onClick={redo} disabled={!canRedo} title="다시 실행 (⌘Y)"
          style={{ ...bStyle(false), opacity: canRedo?1:0.35 }}>↪ 다시</button>

        <div style={{ width:1, height:20, background:border, margin:"0 2px" }} />

        {/* 편집 도구 */}
        <button onClick={addFreeText}              style={bStyle(false)}>+ 텍스트</button>
        <button onClick={()=>addShape("rect")}     style={bStyle(false)}>□</button>
        <button onClick={()=>addShape("circle")}   style={bStyle(false)}>○</button>
        <button onClick={()=>addShape("triangle")} style={bStyle(false)}>△</button>
        <button onClick={()=>addShape("line")}     style={bStyle(false)}>—</button>
        <button onClick={duplicate}                style={bStyle(false)}>복제</button>
        <button onClick={flipH}                    style={bStyle(false)}>반전</button>
        <button onClick={bringFwd}                 style={bStyle(false)}>앞으로</button>
        <button onClick={sendBwd}                  style={bStyle(false)}>뒤로</button>
        <button onClick={removeSelected}           style={{ ...bStyle(false), color:"#E85D2C" }}>삭제</button>

        <div style={{ width:1, height:20, background:border, margin:"0 2px" }} />
        <button onClick={handleToggleLogo} style={bStyle(showLogo)}>로고 {showLogo?"✓":""}</button>

        <div style={{ marginLeft:"auto", display:"flex", gap:8 }}>
          <button onClick={()=>download("png")}
            style={{ ...bStyle(false), background:"#155855", borderColor:"#155855", color:"#fff" }}>
            PNG ↓
          </button>
          <button onClick={()=>download("jpeg")} style={bStyle(false)}>JPG ↓</button>
        </div>
      </div>
      ...
      <div style={{ display:"flex", flex:1, overflow:"hidden" }}>
        ...
        {/* ── 왼쪽 패널 ── */}
        <aside style={{ width:284, background:panelBg, borderRight:`1px solid ${border}`, overflowY:"auto", flexShrink:0 }}>

          {/* 탭 */}
          <div style={{ display:"flex", borderBottom:`1px solid ${border}` }}>
            {(["design","text","filter"] as const).map(tab => (
              <button key={tab} onClick={()=>setActiveTool(tab)}
                style={{ flex:1, height:38, border:"none", background:activeTool===tab?"#222":"transparent",
                         color:activeTool===tab?accent:muted, fontSize:11, fontWeight:700,
                         cursor:"pointer", fontFamily:"inherit",
                         borderBottom:activeTool===tab?`2px solid ${accent}`:"none" }}>
                {tab==="design"?"디자인":tab==="text"?"텍스트·AI":"필터"}
              </button>
            ))}
          </div>
          ...
          {activeTool === "design" && <>
            ...
            <Sec label="사진 업로드" accent={accent}>
              <div
                onClick={()=>fileRef.current?.click()}
                onDragOver={e=>e.preventDefault()}
                onDrop={e=>{ e.preventDefault(); const f=e.dataTransfer.files[0]; if(f) loadImage(f); }}
                style={{ border:`1.5px dashed ${imageLoaded?accent:border}`, borderRadius:9,
                         padding:"14px 8px", textAlign:"center", cursor:"pointer", background:"#1E1E1E" }}>
            ...
            <Sec label="비율" accent={accent}>
              <div style={{ display:"flex", gap:6 }}>
                {RATIOS.map(r=>(
                  <button key={r.key} onClick={()=>handleRatio(r.key)}
                    style={{ flex:1, padding:"8px 4px", border:`1px solid ${ratio===r.key?accent:border}`,
                             borderRadius:8, background:ratio===r.key?"#2A1A10":"#1E1E1E",
                             cursor:"pointer", fontFamily:"inherit" }}>
                    <div style={{ width:r.key==="1:1"?18:r.key==="4:5"?14:10, height:18,
                                  border:`2px solid ${ratio===r.key?accent:"#444"}`,
            ...
            <Sec label="레이아웃" accent={accent}>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:5 }}>
                {TEMPLATES.map(t=>(
                  <button key={t.key} onClick={()=>handleTemplate(t.key)}
                    style={{ padding:"8px 7px", border:`1px solid ${template===t.key?accent:border}`,
                             borderRadius:8, background:template===t.key?"#2A1A10":"#1E1E1E",
            ...
            <Sec label="액센트 컬러" accent={accent}>
              {[PC_STYLE.brand.orange,PC_STYLE.brand.teal,PC_STYLE.brand.orange2,PC_STYLE.brand.teal2,"#FFFFFF","#1C2B28"].map(hex=>(
                <span key={hex} onClick={()=>setAccentColor(hex)}
                  style={{ display:"inline-block", width:24, height:24, borderRadius:"50%", background:hex,
                           border:`2px solid ${accentColor===hex?"#fff":"transparent"}`,
                           cursor:"pointer", margin:"0 4px 4px 0",
                           transform:accentColor===hex?"scale(1.2)":"scale(1)", transition:"all .15s",
                           boxShadow:hex==="#FFFFFF"?"inset 0 0 0 1px #444":"none" }} />
              ))}
            ...
            <Sec label="팬톤 컬러 팔레트" accent={accent}>
              <button onClick={()=>setShowPantone(v=>!v)} style={{ ...bStyle(showPantone), width:"100%", marginBottom:8 }}>
                {showPantone?"팔레트 닫기 ▲":"팬톤 컬러 추천 ▼"}
              </button>
              {showPantone && <>
                <div style={{ display:"flex", gap:4, marginBottom:8, flexWrap:"wrap" }}>
                  {PANTONE_PALETTES.map((p,i)=>(
                    <button key={p.name} onClick={()=>setActivePalette(i)}
                      style={{ fontSize:9, padding:"3px 7px", border:`1px solid ${activePalette===i?accent:border}`,
                               borderRadius:5, background:activePalette===i?"#2A1A10":"#1E1E1E",
                               color:activePalette===i?accent:muted, cursor:"pointer", fontFamily:"inherit" }}>
                      {p.name}
                    </button>
                  ))}
                </div>
                <div style={{ display:"flex", gap:4 }}>
                  {PANTONE_PALETTES[activePalette].colors.map(c=>(
                    <div key={c.hex} onClick={()=>{ setTextColor(c.hex); setAccentColor(c.hex); }}
                      title={`${c.name}\n${c.label}`}
                      style={{ flex:1, height:38, borderRadius:6, background:c.hex, cursor:"pointer",
                               border:`2px solid ${textColor===c.hex?"#fff":"transparent"}`, transition:"all .15s" }}/>
                  ))}
                </div>
                <div style={{ display:"flex", gap:4, marginTop:4 }}>
                  {PANTONE_PALETTES[activePalette].colors.map(c=>(
                    <div key={c.hex} style={{ flex:1, textAlign:"center", fontSize:8, color:muted, lineHeight:1.3 }}>{c.label}</div>
                  ))}
                </div>
                <div style={{ fontSize:9, color:muted, marginTop:6 }}>클릭 → 텍스트·액센트 컬러 적용</div>
              </>}
            </Sec>
          </>}
          ...
          {activeTool === "text" && <>
            ...
            <Sec label="콘텐츠 유형" accent={accent}>
              <div style={{ display:"flex", flexWrap:"wrap", gap:5 }}>
                {CONTENT_TYPES.map(ct=>(
                  <button key={ct.key} onClick={()=>setContentType(ct.key)}
                    style={{ padding:"6px 8px", border:`1px solid ${contentType===ct.key?accent:border}`,
                             borderRadius:7, background:contentType===ct.key?"#2A1A10":"#1E1E1E",
                             color:contentType===ct.key?accent:txt, fontSize:11, fontWeight:600,
                             cursor:"pointer", fontFamily:"inherit" }}>
                    {ct.emoji} {ct.label}
                  </button>
                ))}
              ...
              {activeTab === "ai" ? (
                <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
                  ...
                  <button onClick={generateCaption} disabled={generating}
                    style={{ height:38, background:generating?"#333":accent, color:"#fff", border:"none",
                             borderRadius:8, fontSize:12, fontWeight:700,
                             cursor:generating?"not-allowed":"pointer", fontFamily:"inherit",
                             display:"flex", alignItems:"center", justifyContent:"center", gap:7 }}>
                    {generating ? <><Spin/><span>생성 중...</span></> : "스토리 캡션 생성 (5줄+)"}
                  </button>
                  {isMock && (
                    <div style={{ fontSize:10, color:"#C8860A", background:"#2A2000", borderRadius:6, padding:"5px 8px" }}>
                      샘플 · ANTHROPIC_API_KEY 설정 시 실제 생성
                    </div>
                  )}
                  {captions.length > 0 && <>
                    {captions.map((cap,i)=>(
                      <div key={i} onClick={()=>setSelIdx(i)}
                        style={{ border:`1px solid ${selIdx===i?accent:border}`, borderRadius:8,
                                 padding:"8px 10px", cursor:"pointer",
                                 background:selIdx===i?"#2A1A10":"#1E1E1E" }}>
                        <div style={{ fontSize:9, fontWeight:700, color:accent, marginBottom:3 }}>{cap.type}</div>
                        <div style={{ fontSize:11, lineHeight:1.65, color:txt, whiteSpace:"pre-line",
                                      maxHeight:110, overflowY:"auto" }}>{cap.text}</div>
                      </div>
                    ))}
                    <div style={{ fontSize:10, color:"#569082", lineHeight:1.7, wordBreak:"break-all", padding:"4px 0" }}>
                      {hashtags}
                    </div>
                    <button onClick={()=>placeText(captions[selIdx].text)}
                      style={{ height:34, background:"#155855", color:"#fff", border:"none",
            ...
            <Sec label="폰트 & 크기" accent={accent}>
              <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                {FONT_PAIRS.map((fp,i)=>(
                  <button key={i} onClick={()=>setFontPair(i)}
                    style={{ padding:"7px 10px", border:`1px solid ${fontPair===i?accent:border}`,
                             borderRadius:7, background:fontPair===i?"#2A1A10":"#1E1E1E",
                             color:fontPair===i?accent:txt, fontSize:11, cursor:"pointer",
                             fontFamily:"inherit", textAlign:"left" }}>
                    <span style={{ fontFamily:`'${fp.display}',serif,sans-serif`, marginRight:6 }}>
                      {fp.display.split(" ")[0]}
                    </span>
                ...
                {[
                  { label:`${fontSize}px`, min:12, max:80, val:fontSize, set:setFontSize },
                  { label:`줄간격 ${lineH.toFixed(1)}`, min:10, max:25, val:Math.round(lineH*10), set:(v:number)=>setLineH(v/10) },
                  { label:`자간 ${letterSp}`, min:-5, max:20, val:letterSp, set:setLetterSp },
                ].map(({ label, min, max, val, set })=>(
                  <div key={label} style={{ display:"flex", alignItems:"center", gap:7, marginTop:4 }}>
                    <label style={{ fontSize:10, color:muted, minWidth:55 }}>{label}</label>
                ...
                <div style={{ display:"flex", gap:5, marginTop:4 }}>
                  {(["left","center","right"] as const).map(a=>(
                    <button key={a} onClick={()=>setTextAlign(a)}
                      style={{ flex:1, height:28, border:`1px solid ${textAlign===a?accent:border}`,
                               borderRadius:6, background:textAlign===a?"#2A1A10":"#1E1E1E",
                               color:textAlign===a?accent:muted, fontSize:12, cursor:"pointer" }}>
                      {a==="left"?"←":a==="center"?"≡":"→"}
                    </button>
                  ))}
                </div>
            ...
            <Sec label="텍스트 색상" accent={accent}>
                ...
                {["#FFFFFF","#F5F0EB","#E8E4DC","#E85D2C","#EB8F22","#155855","#569082","#1C2B28","#000000"].map(hex=>(
                  <div key={hex} onClick={()=>handleTextColor(hex)}
                    style={{ width:24, height:24, borderRadius:"50%", background:hex,
                             border:`2px solid ${textColor===hex?"#fff":"transparent"}`,
                             cursor:"pointer", transform:textColor===hex?"scale(1.2)":"scale(1)",
                             transition:"all .15s", boxShadow:hex==="#FFFFFF"?"inset 0 0 0 1px #444":"none" }}/>
                ))}
              </div>
              <div style={{ marginTop:8, display:"flex", alignItems:"center", gap:6 }}>
                <label style={{ fontSize:10, color:muted }}>커스텀</label>
          ...
          {/* ── 필터 탭 ── */}
          {activeTool === "filter" && <>
            <Sec label="사진 필터" accent={accent}>
              <div style={{ fontSize:10, color:muted, marginBottom:10, lineHeight:1.6 }}>
                포토클리닉 프리셋: 따뜻한 필름 느낌<br/>
                밝기 -7 · 대비 +2 · 채도 -18 · 색온도 +20
              </div>
              {[
                { label:"밝기",   val:brightness, set:setBrightness, min:50, max:150, def:93  },
                { label:"대비",   val:contrast,   set:setContrast,   min:50, max:150, def:102 },
                { label:"채도",   val:saturation, set:setSaturation, min:0,  max:200, def:82  },
                  ...
                  <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                    ...
                    <div style={{ display:"flex", gap:6, alignItems:"center" }}>
                      ...
                      <button onClick={()=>set(def)}
                        style={{ fontSize:9, color:muted, background:"none", border:`1px solid ${border}`,
                                 borderRadius:4, padding:"1px 5px", cursor:"pointer" }}>초기화</button>
                    </div>
                  </div>
                  <input type="range" min={min} max={max} value={val}
                    onChange={e=>set(+e.target.value)} style={{ width:"100%", accentColor:accent }}/>
                </div>
              ))}
              <button
                onClick={()=>{ setBrightness(93); setContrast(102); setSaturation(82); setWarmth(20); }}
                style={{ ...bStyle(false), width:"100%", marginTop:4, borderColor:accent, color:accent }}>
                🎞 포토클리닉 필름 프리셋 적용
              </button>
              <button
                onClick={()=>{ setBrightness(93); setContrast(102); setSaturation(82); setWarmth(0); }}
                style={{ ...bStyle(false), width:"100%", marginTop:6 }}>
                기본값으로 초기화
              </button>
            </Sec>
          </>}
        ...
        <main style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center",
                       justifyContent:"flex-start", padding:"24px", overflowY:"auto", background:"#0D0D0B" }}>
      ...
      <div style={{
        ...
        background:"#155855", color:"#fff", fontSize:12, padding:"9px 18px",
...
function Sec({ label, children, accent }: { label:string; children:React.ReactNode; accent:string }) {
  return (
    <div style={{ marginBottom:18, paddingBottom:16, borderBottom:"1px solid #222" }}>
      <div style={{ fontSize:9, fontWeight:700, color:"#555", textTransform:"uppercase",
...
function Spin({ size=14 }: { size?:number }) {
  return (
    <div style={{ width:size, height:size,
                  border:`${Math.max(2,Math.floor(size/7))}px solid rgba(255,255,255,.15)`,
                  borderTopColor:"#E85D2C", borderRadius:"50%",
